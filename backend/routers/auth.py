...
def register(
    ...
    if username_error:
        raise HTTPException(status_code=400, detail=username_error)  # Simple string
    
    email_error = validate_email(email)
    if email_error:
        raise HTTPException(status_code=400, detail=email_error)  # Simple string
    
    password_error = validate_password(password)
    if password_error:
        raise HTTPException(status_code=400, detail=password_error)  # Simple string
